...
class SMILESEmbedding(nn.Module):
    ...
    def __init__(self, smiles_vocabulary_path, smiles_embedding_size, smiles_files=None, scale_grad_by_freq=False):
        """
        SMILES-Einbettung
        :param smiles_vocabulary_path: Pfad zur Vokabular-Datei (.pkl).
        :param smiles_embedding_size: Größe der Embedding-Vektoren.
        :param smiles_files: Optional: Liste von SMILES-Dateien, um das Vokabular zu erstellen.
        ...
        # Wenn Vokabular nicht existiert, erstelle es
        if not os.path.exists(smiles_vocabulary_path):
            if smiles_files is None:
                raise ValueError("SMILES-Dateien müssen angegeben werden, um das Vokabular zu erstellen.")
    ...
    def load_vocab(self, vocab_path):
        """
        Lade das SMILES-Vokabular aus einer .pkl-Datei.
        :param vocab_path: Pfad zur Vokabular-Datei.
    ...
    def create_vocab(self, smiles_files):
        """
        Erstelle das Vokabular aus den angegebenen SMILES-Dateien.
        ...
        for smiles_file in smiles_files:
            with open(smiles_file, 'r') as f:
                for line in f:
                    for char in line.strip():  # Hier könnte auch ein komplexerer Tokenizer verwendet werden
                        vocab.add(char)
    ...
    def tokenize_and_integerize(self, smiles):
        """
        Tokenisiere eine SMILES-Zeichenkette und wandle sie in Integer um.
        :param smiles: SMILES-Zeichenkette.
        :return: Liste von Indizes.
        """
        tokens = list(smiles)  # Hier kann ein komplexerer Tokenizer verwendet werden
    ...
    def save_to_csv(self, processed_smiles, file_path):
        """
        Speichert verarbeitete SMILES in eine CSV-Datei.
        :param processed_smiles: Tensor mit integerisierten und gepaddeten SMILES.
        :param file_path: Pfad zur Ausgabedatei.
        """
        # Konvertiere den Tensor in ein numpy-Array und speichere ihn als DataFrame
    ...
    def forward(self, smiles_indices):
        """
        Führt das Einbetten für die gegebenen SMILES-Indizes aus.
